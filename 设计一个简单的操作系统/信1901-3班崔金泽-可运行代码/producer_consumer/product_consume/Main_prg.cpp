...
const unsigned short p_count = 5;        //produce 一次的数量
const unsigned short c_count = 5;        //consumer 一次的数量
const unsigned short s_count = p_count + c_count;  //线程的数量和
bool control = true;
HANDLE threads[s_count];               //the handle of every thread
DWORD Producer_threadID[p_count];            //标记 producer thread
DWORD Consumer_threadID[c_count];            //标记 consumer thread
